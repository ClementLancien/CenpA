...
class Download():
	...
	def get_all_study(self):
		...
		index_sample_title = headers.index('sample_title')
		index_fastq_ftp = headers.index('fastq_ftp')
		index_read_count = headers.index('read_count')

		df = df.iloc[[0,1,2,3],[index_sample_title, index_fastq_ftp, index_read_count]]

		for i in range(df.shape[0]):

			sample_title = df.iloc[i,0].replace(' ','_').replace('(','').replace(')','')
			links = df.iloc[i,1].split(';')

			for link in links:
				output = os.path.join(self.path_study, sample_title + '_' + link.rsplit('/',1)[1])
				self.get_study(link, output)


	def get_study(self, link, output):

		try:
			# print link
			# print "wget -nc " + link + " --output-document= " + output + " &>/dev/null"
			subprocess.check_output(['bash', '-c', "wget -nc " + link + " --output-document=" + output + " &>/dev/null"])

		except subprocess.CalledProcessError as error:
			print "here"
			self.logger.warning("Error - download.py - get_study " + str(output.rsplit('/',1))[1])
			self.logger.warning("Exception at the line : {}".format(sys.exc_info()[-1].tb_lineno))
			self.logger.warning(sys.exc_info())
